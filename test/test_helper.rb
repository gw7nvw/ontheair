...
VCR.configure do |config|
  # Where VCR will store recorded API payloads as YAML files
  config.cassette_library_dir = "test/vcr_cassettes"
  config.hook_into :webmock

  # Crucial for Rails: allows local integration tests (Capybara/System tests) to keep working
...
Band.create({"meter_band"=>"80m", "freq_band"=>"3.6MHz", "group"=>"HF", "min_frequency"=>3.5, "max_frequency"=>4.0})
  ...
  def create_test_log(user, params={})
     ...
     params[:createdBy] = User.first if !params[:createBy]
  ...
  def transform_handsontable_payload(legacy_matrix)
    formatted_payload = {}

    legacy_matrix.each_with_index do |row, row_idx|
      row_hash = {}

      row.each_with_index do |value, col_idx|
        # Coerce values to mimic standard browser JSON/AJAX serialization
        row_hash[col_idx.to_s] = case value
                                 when true  then "true"
                                 when false then "false"
                                 when nil   then ""
                                 else value # Strings, Numbers, and Nested Arrays stay as they are
                                 end
      end
  
      formatted_payload[row_idx.to_s] = row_hash
    end
