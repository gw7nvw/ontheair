...
class ContactChildrenTest < ActiveSupport::TestCase
  ...
  test "vk_assets use lookup table to find contained_by_assets" do
    ...
    log=create_test_log(user1)
    contact=create_test_contact(user1, user2, log_id: log.id, asset2_codes: [asset3.code])
  ...
  test "master codes applied for inactive asset" do
    ...
    assert contact.asset2_codes.sort==[asset1.code].sort, "Inactive park replaced with master replacement"
  end
  ...
  test "inactive contained_by_assets not applied" do
    ...
    log=create_test_log(user1)
    contact=create_test_contact(user1, user2, log_id: log.id, asset2_codes: [asset2.code])
    assert contact.asset2_codes.sort==[asset2.code].sort, "Inactive chid park not applied even though it contains parent"
  end
