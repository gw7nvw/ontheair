...
class ContactSaveTest < ActiveSupport::TestCase
  ...
  test "asset1 codes inherited from log" do
    ...
    contact=create_test_contact(user1, user2, log_id: log.id)
    assert contact.asset1_codes.sort==[asset1.code, asset2.code].sort, "Asset 1 codes inherited from log"
  end
  ...
  test "asset2 codes can be read from loc_desc2 if not already present" do
    ...
    assert contact.asset2_codes.sort==[asset1.code].sort, "Asset 2 code read from loc_desc2"
    #multiple codes
    contact2=create_test_contact(user1, user2, log_id: log.id, loc_desc2:asset1.code+","+asset2.code)
    assert contact2.asset2_codes.sort==[asset1.code,asset2.code].sort, "Multiple asset 2 codes read from loc_desc2"

    #not a code
    contact3=create_test_contact(user1, user2, log_id: log.id, loc_desc2: "Hello world")
    assert contact3.asset2_codes.sort==[].sort, "Can ignore text descriptions in loc_desc2"

    #a code from another award programme
    contact4=create_test_contact(user1, user2, log_id: log.id, loc_desc2: "KFF-0001")
    assert contact4.asset2_codes.sort==["KFF-0001"].sort, "Code from another award programme accepted"
    ...
    assert contact5.asset2_codes.sort==[asset1.code].sort, "Ignore code in description if one provided in asset2_codes"
  end
  ...
  test "Cannot activate and chase same location" do
    ...
    #single code in desc
    contact=create_test_contact(user1, user2, log_id: log.id, loc_desc2: asset1.code)
    assert contact.asset2_codes==[], "Asset 2 code ignored when is duplicate of asset1_code"
    assert contact.loc_desc2=='INVALID', "Lod_desc overwritten when it contains invalid code"

    #single code in codes
    contact2=create_test_contact(user1, user2, log_id: log.id, asset2_codes: [asset1.code])
    assert contact2.asset2_codes==[], "Asset 2 code ignored when is duplicate of asset1_code"
    assert contact2.loc_desc2=='INVALID', "Lod_desc overwritten when it contains invalid code"

    #multiple code in codes
