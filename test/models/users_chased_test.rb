...
class UserChasedTest < ActiveSupport::TestCase
  ...
  test "actiuvating log triggers chase for chasing party (only)" do
    ...
    assert_equal user1.chased, [], "Activating user has not chased location 1"
    assert_equal user2.chased, [asset1.code], "Chasing user has chased location 1"
  end
  ...
  test "chaser log triggers chase for chasing party only" do
    ...
    assert_equal user1.chased, [asset1.code], "Chasing user has chased location 1"
    assert_equal user2.chased, [], "Activating user has not chased location 1"
  end
  ...
  test "single chase listed if both patries log the contact" do
    ...
    assert_equal user1.chased, [], "Activating user has not chased location 1"
    assert_equal user2.chased, [asset1.code], "Chasing user has chased location 1 only once"
  end
  ...
  test "Can request specific asset types" do
    ...
    assert_equal user2.chased(asset_type: 'hut'), [asset1.code], "Chasing user has chased this hut: "+user2.chased(asset_type: 'hut').to_json
    assert_equal user2.chased(asset_type: 'park'), [], "Chasing user has chased no parks"
    assert_equal user1.chased(asset_type: 'hut'), [], "Activating user has not chased this hut"
  end
  ...
  test "Can request multiple asset types" do
    ...
    assert_equal user2.chased(asset_type: 'hut, park').sort, [asset1.code, asset2.code].sort, "Chasing user has chased hut and park: "+user2.chased(asset_type: 'hut, park').to_json
    assert_equal user2.chased(asset_type: 'island, lighthouse').sort, [], "Chasing user has not chased island or lighthouse: "+user2.chased(asset_type: 'island, lighthouse').to_json
  end
  ...
  test "minor assets not included unless requested" do 
    ...
    assert_equal user1.chased, [], "Activating user does not list minor asset"
    assert_equal user2.chased, [], "Chasing user does not list minor asset"

    assert_equal user1.chased(include_minor: true), [], "Activating user does not lost this asset even with minor requested"
    assert_equal user2.chased(include_minor: true), [asset1.code], "Chasing user has activated location 1 when minor is requested"
  end
  ...
  test "QRO contacts not included if QRP requested" do 
    ...
    assert_equal user2.chased(qrp: true), [], "Chasing user does not list non-QRP contact (all)"
    assert_equal user2.chased(qrp: true, asset_type: 'hut'), [], "Chasing user does not list non-QRP contact (hut)"

    assert_equal user2.chased(qrp: false), [asset1.code], "Can pass qrp=false as parameter"
    ...
    assert_equal user2.chased(qrp: true), [asset1.code], "Chasing user lists QRP (party1) contact (all)"
    assert_equal user2.chased(qrp: true, asset_type: 'hut'), [asset1.code], "Chasing user lists QRP (party1) contact (hut)"

    contact.is_qrp1=true
    contact.is_qrp2=false
    contact.save

    assert_equal user2.chased(qrp: true), [], "Chasing user does not list QRP (party2) contact (all)"
    assert_equal user2.chased(qrp: true, asset_type: 'hut'), [], "Chasing user does not list QRP (party2) contact (hut)"

    contact.is_qrp1=false
    contact.is_qrp2=true
    contact.save

    assert_equal user2.chased(qrp: true), [asset1.code], "Chased user does list QRP (party2) contact (all)"
    assert_equal user2.chased(qrp: true, asset_type: 'hut'), [asset1.code], "Chased user does list QRP (party2) contact (hut)"
  end
  ...
  test "Muliple references in an activation are all picked up in activations" do
    ...
    assert_equal user2.chased.sort, [asset1.code,asset2.code].sort, "Chasing user has chased both locations"
    assert_equal user1.chased, [], "Activating user not activated both locations"
  end
  ...
  test "Multiple chases of same reference generate only one all-time chase entry" do
    ...
    assert_equal user2.chased, [asset1.code], "Chasing user sees multiply-chased location only once"
  end
  ...
  test "Multiple chases by_year in different UTC year listed twice" do
    ...
    assert_equal user2.chased(by_year: true).sort, [asset1.code+" 2019", asset1.code+" 2020"], "Chasing user sees multiply-chased location twice for different years"
  end
  ...
  test "Multiple chases by_year in same UTC year listed once" do
    ...
    assert_equal user2.chased(by_year: true), [asset1.code+" 2020"], "Chasing user sees multiply-chased location once for same year"
  end
  ...
  test "Multiple chases by_day in different UTC day listed twice" do
    ...
    assert_equal user2.chased(by_day: true).sort, [asset1.code+" 2020-01-01", asset1.code+" 2020-01-02"], "Chasing user sees multiply-chased location twice for different days"
  end
  ...
  test "Multiple chases by_day in same UTC day listed once" do
    ...
    assert_equal user2.chased(by_day: true), [asset1.code+" 2020-01-01"], "Chasing user sees multiply-chased location once for same day"
  end
  ...
  test "Multiple copies of reference in an chase generate only one entry" do
    ...
    assert_equal user2.chased.sort, [asset1.code,asset2.code].sort, "Chasing user has chased both locations but repeated locn shown only once"
  end
  ...
  test "Chase using secondary callsign outside time not picked up in chased" do
    ...
    assert_equal uc.callsign, user2.callsign, "User1 callsign applied to user3 as secondary call"
    ...
    assert_equal contact.callsign2, uc.callsign, "Secondary call applied to contact"
    assert_equal user3.chased, [asset1.code], "Chasing call with correct dates has chased location 1"
    assert_equal user2.chased, [], "Another user with same call different dates to chaser has not chased location 1"
  end
