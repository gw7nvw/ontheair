...
class UserOrphanActivationTest < ActiveSupport::TestCase
  ...
  test "single activation listed if both partys log the contact" do
    user1=create_test_user
    user2=create_test_user
    asset1=create_test_asset
  ...
  test "only_activator returns only activator logs, ignores chaser logs" do
    user1=create_test_user
    user2=create_test_user
    asset1=create_test_asset
    asset2=create_test_asset
    ...
    #Chaser log only
    log2=create_test_log(user2)
    contact2=create_test_contact(user2,user1,log_id: log2.id, asset2_codes: [asset2.code])
    #logged by both activator and chaser
    log3=create_test_log(user1,asset_codes: [asset3.code])
    contact2=create_test_contact(user1,user2,log_id: log3.id, asset1_codes: [asset3.code])
    log4=create_test_log(user2)
    contact2=create_test_contact(user2,user1,log_id: log4.id, asset2_codes: [asset3.code])

    codes=user1.activations
    assert_equal codes.sort, [asset1.code, asset2.code, asset3.code], "By default list activations from both activator and chaser logs"
    codes=user1.activations(only_activator: true)
    assert_equal codes.sort, [asset1.code, asset3.code], "Can request  just activator logs"
    codes=user1.activations(orphan_activations: true)
    assert_equal codes.sort, [asset2.code], "Can request items missing from activator log that are in chaser logs"
  end

  test "only_activator returns only activator logs, ignores chaser logs (by year)" do
    user1=create_test_user
    user2=create_test_user
    asset1=create_test_asset
    asset2=create_test_asset
    ...
    contact2=create_test_contact(user2,user1,log_id: log2.id, asset2_codes: [asset2.code], time: '2022-01-01 01:03'.to_time)
    #logged by both activator and chaser
    log3=create_test_log(user1,asset_codes: [asset3.code], date: '2022-01-01'.to_date)
    contact2=create_test_contact(user1,user2,log_id: log3.id, asset1_codes: [asset3.code], time: '2022-01-01 01:04'.to_time)
    log4=create_test_log(user2, date: '2022-01-01'.to_date)
    contact2=create_test_contact(user2,user1,log_id: log4.id, asset2_codes: [asset3.code], time: '2022-01-01 01:05'.to_time)

    codes=user1.activations(by_year: true)
    assert_equal codes.sort, [asset1.code+" 2022", asset2.code+" 2022", asset3.code+" 2022"], "By default list activations from both activator and chaser logs"
    codes=user1.activations(only_activator: true, by_year: true)
    assert_equal codes.sort, [asset1.code+" 2022", asset3.code+" 2022"], "Can request  just activator logs"
    codes=user1.activations(orphan_activations: true, by_year: true)
    assert_equal codes.sort, [asset2.code+" 2022"], "Can request items missing from activator log that are in chaser logs"
  end
end
