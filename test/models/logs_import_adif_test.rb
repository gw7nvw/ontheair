...
class LogImportAdifTest < ActiveSupport::TestCase
  ...
  test "Log contact imported successfully" do
    ...
    assert logs[:success]==true, "Sucessful exit"
    assert logs[:errors]==[], "No errors"
    assert logs[:good_logs]==1, "One log"
    assert logs[:good_contacts]==1, "one contact"
    assert logs[:logs].count==1, "Really is one log"
    log=logs[:logs].first

    assert log.user1_id==user1.id
    assert log.callsign1==user1.callsign
    assert log.asset_codes==[asset1.code]
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts.first
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset1.code]
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[asset2.code]
    assert contact.comments1=="This is a comment"
    assert contact.mode=='FM'
    assert contact.frequency==144
  end
  ...
  test "Log imports multiple contacts successfully" do
    ...
    assert logs[:errors]==[], "No errors"
    assert logs[:good_logs]==1, "One log"
    assert logs[:good_contacts]==2, "two contacts"
    assert logs[:logs].count==1, "Really is one log"
    log=logs[:logs].first
    ...
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==2, "Really is two contact"
    contact=contacts[0]
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset1.code]
    ...
    assert contact.asset1_codes==[asset1.code]
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:05'.to_time
    assert contact.user2_id=user3.id
    assert contact.callsign2==user3.callsign
    assert contact.asset2_codes==[asset3.code]
    assert contact.comments1=="This is a comment2"
  end
  ...
  test "Can handle YYYY-mm-dd as well as dd-mm-yy" do
    ...
    assert logs[:errors]==[], "No errors"
    assert logs[:good_logs]==1, "One log"
    assert logs[:good_contacts]==1, "One contacts"
    assert logs[:logs].count==1, "Really is one log"
    log=logs[:logs].first
    assert log.date=='2024-08-01'.to_date
    contact=log.contacts.first
    assert contact.date=='2024-08-01'.to_date
    assert contact.time=='2024-08-01 23:04'.to_time
  end
  ...
  test "Can handle minimum values" do
    ...
    assert contact.asset1_codes==[asset1.code]
    assert contact.date=='2024-08-01'.to_date
    assert contact.time=='2024-08-01 00:00'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[]
    assert contact.comments1==nil
  end
  ...
  test "can crete new user2" do
    ...
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=User.last.id
    assert contact.user2_id!=last_user_id, 'new user has been created'
    assert contact.callsign2=='VK3AAA'
    assert contact.asset2_codes==[asset2.code]
    assert contact.comments1=="This is a comment"
  end
  ...
  test "can handle unknown assets" do
    ...
    assert log.user1_id==user1.id
    assert log.callsign1==user1.callsign
    assert log.asset_codes==["KFF-0001"]
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts.first
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==["KFF-0001"]
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==["GM/SE-001"]
    assert contact.comments1=="This is a comment"
  end
  ...
  test "rejects contact with no date" do
    ...
    assert logs[:success]==true, "Sucessful exit"
    assert logs[:errors].count>0, "Error(s) reportred"
    assert logs[:good_logs]==0, "No good logs"
    assert logs[:good_contacts]==0, "no good contacts"
    assert logs[:logs].count==0, "No log"
  end
  ...
  test "rejects contact for another call" do
    ...
    assert logs[:success]==true, "Sucessful exit"
    assert logs[:errors].count==1, "Error reportred"
    assert logs[:good_logs]==0, "No good logs"
    assert logs[:good_contacts]==0, "no good contacts"
    assert logs[:logs].count==0, "No logs"
  end
  ...
  test "continue after bad contact entry" do
    ...
    assert logs[:success]==true, "Sucessful exit"
    assert logs[:errors].count==1, "Error reportred"
    assert logs[:good_logs]==1, "One good logs"
    assert logs[:good_contacts]==1, "One good contacts"
    log=logs[:logs][0]
    assert log.id!=nil, "First log is saved"
    assert log.user1_id==user1.id
    assert log.callsign1==user1.callsign
    assert log.asset_codes==[asset1.code]
    assert log.date=='2024-08-01'.to_date
    ...
    assert contact.asset1_codes==[asset1.code]
    assert contact.date=='2024-08-01'.to_date
    assert contact.time=='2024-08-01 23:05'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[asset2.code]
    assert contact.comments1=="This is a comment2"
  ...
  test "allows contacts for secondary call" do
    ...
    assert log.user1_id==user1.id
    assert log.callsign1==uc1.callsign
    assert log.asset_codes==[asset1.code]
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts.first
    assert contact.user1_id==user1.id
    assert contact.callsign1==uc1.callsign
    assert contact.asset1_codes==[asset1.code]
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==uc2.callsign
    assert contact.asset2_codes==[asset2.code]
    assert contact.comments1=="This is a comment"
  end
  ...
  test "allows chaser log" do
    ...
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[]
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[asset1.code]
    assert contact.comments1=="This is a comment"
    assert contact.mode=='FM'
    assert contact.frequency==144
    contact=contacts[1]
    ...
    assert contact.asset2_codes==[asset1.code]
    assert contact.comments1=="This is a comment"
    assert contact.mode=='AM'
    assert contact.frequency==7
  end
  ...
  test "asset inherits child assets when requested" do
    ...
    assert log.user1_id==user1.id
    assert log.callsign1==user1.callsign
    assert log.asset_codes==[asset1.code, asset2.code]
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts.first
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset1.code, asset2.code]
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[asset3.code, asset4.code]
    assert contact.comments1=="This is a comment"
    assert contact.mode=='FM'
    assert contact.frequency==144
  end
  ...
  test "asset does not inherit child assets when do_not_lookup requested" do
    ...
    assert log.user1_id==user1.id
    assert log.callsign1==user1.callsign
    assert log.asset_codes==[asset1.code], "Did not inherit child code"
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts.first
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset1.code], "Did not inherit child code"
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[asset3.code, asset4.code], "chaser does inherit"
    assert contact.comments1=="This is a comment"
    assert contact.mode=='FM'
    assert contact.frequency==144
  end
  ...
  test "can specify callsign and location (none in log)" do
    ...
    assert log.user1_id==user1.id
    assert log.callsign1==uc1.callsign
    assert log.asset_codes==['VKFF-0001']
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts.first
    assert contact.user1_id==user1.id
    assert contact.callsign1==uc1.callsign
    assert contact.asset1_codes==['VKFF-0001']
    assert contact.date=='2024-08-30'.to_date
    assert contact.time=='2024-08-30 23:04'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
  ...
  test "logs broken down by day and location1" do
    ...
    assert logs[:success]==true, "Sucessful exit"
    assert logs[:errors]==[], "No errors"
    assert logs[:good_logs]==3, "Three log"
    assert logs[:good_contacts]==4, "two contacts"
    assert logs[:logs].count==3, "Really is three log"
    ...
    assert log.callsign1==user1.callsign
    assert log.asset_codes==[asset1.code]
    assert log.date=='2024-08-29'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts[0]
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset1.code]
    assert contact.date=='2024-08-29'.to_date
    assert contact.time=='2024-08-29 00:00'.to_time
    assert contact.user2_id=user2.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[asset2.code]
    assert contact.comments1=="This is a comment"
    ...
    assert log.user1_id==user1.id
    assert log.callsign1==user1.callsign
    assert log.asset_codes==[asset2.code], "New location, new log"
    assert log.date=='2024-08-29'.to_date
    contacts=log.contacts
    assert contacts.count==1, "Really is one contact"
    contact=contacts[0]
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset2.code]
    assert contact.date=='2024-08-29'.to_date
    assert contact.time=='2024-08-29 06:00'.to_time
    assert contact.user2_id=user3.id
    assert contact.callsign2==user3.callsign
    assert contact.asset2_codes==[asset3.code]
    assert contact.comments1=="This is a comment2"
    ...
    assert log.user1_id==user1.id
    assert log.callsign1==user1.callsign
    assert log.asset_codes==[asset2.code]
    assert log.date=='2024-08-30'.to_date
    contacts=log.contacts
    assert contacts.count==2, "Really is two contact"
    contact=contacts[0]
    ...
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset2.code]
    assert contact.date=='2024-08-30'.to_date, "New date, new log"
    assert contact.time=='2024-08-30 06:00'.to_time
    assert contact.user2_id=user3.id
    assert contact.callsign2==user2.callsign
    assert contact.asset2_codes==[asset3.code]
    assert contact.comments1=="This is a comment3"
    assert contact.mode=='SSB'
    assert contact.frequency==14
    contact=contacts[1]
    assert contact.user1_id==user1.id
    assert contact.callsign1==user1.callsign
    assert contact.asset1_codes==[asset2.code], "Same asset, same log"
    assert contact.date=='2024-08-30'.to_date, "Same day, same log"
    assert contact.time=='2024-08-30 07:00'.to_time
    assert contact.user2_id=user3.id
    assert contact.callsign2==user3.callsign
    assert contact.asset2_codes==[asset3.code]
    assert contact.comments1=="This is a comment4"
    assert contact.mode=='AM'
    assert contact.frequency==7
  end
end
