...
class ApiPnpAlertTest < ActionDispatch::IntegrationTest
  ...
  # /api/ALERT
  ##################################################################
  test "Should get api/ALERT alerts index" do
    user1=create_test_user
    user2=create_test_user
    asset1=create_test_asset(asset_type: 'hut', region: 'OT', location: create_point(169.1,-45.2))
    asset2=create_test_asset(asset_type: 'wwff park', region: 'OT', location: create_point(169.1,-45.2), test_radius: 0.1, code_prefix: 'ZLFF-0')
    asset3=create_test_asset(asset_type: 'pota park', region: 'OT', location: create_point(169.1,-45.2), test_radius: 0.2, code_prefix: 'NZ-0')

    t1 = 2.minutes.ago.floor
    t2 = 1.minute.ago.floor

    item1=create_test_alert(user1, asset_codes: [asset1.code, asset2.code], callsign: user2.callsign, referenced_time: t1, freq: 7.09, mode: "SSB", duration: 1, description: "This is a comment")

    item2=create_test_alert(user1, asset_codes: [asset3.code], callsign: user1.callsign, referenced_time: t2, freq: 7.09, mode: "SSB", duration: 2, description: "This is a comment")
    get "/api/ALERTS.json"

    assert_response :success

    data = JSON.parse(@response.body)
    data = data.sort_by { |d| d["atID"] }
    assert_equal data.count, 2
    assert_equal data[1].excluding(["alID"]), 
      {"WWFFID" => asset1.code, "actSiteID" => asset1.code, "allAssetCodes" => [asset1.code, asset2.code, asset3.code], "CallSign" => user2.callsign, "Class" => "ZLOTA", "Location" => "#{asset1.name} [#{asset1.code}] {RE44nt}; #{asset2.name} [#{asset2.code}] {RE44nt}; #{asset3.name} [#{asset3.code}] {RE44nt}", "alDay" => "0", "alTime" => t1.strftime("%Y-%m-%d %H:%M:%S"), "Freq" => "7.09", "MODE" => "SSB", "Comments" => "This is a comment", "Duration" => "1"}
      "first row should match"
    assert_equal data[0].excluding(["alID"]), 
      {"WWFFID" => asset3.code, "actSiteID" => asset3.code, "allAssetCodes" => [asset3.code], "CallSign" => user1.callsign, "Class" => "POTA", "Location" => "#{asset3.name} [#{asset3.code}] {RE44nt}", "alDay" => "0", "alTime" => t2.strftime("%Y-%m-%d %H:%M:%S"), "Freq" => "7.09", "MODE" => "SSB", "Comments" => "This is a comment", "Duration" => "2"}
