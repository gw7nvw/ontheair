...
class PasswordResetsControllerTest < ActionController::TestCase
  ...
  test "Can request password reset" do
    ...
    post :create, params: {password_reset: {email: "reset_test@test.com"}}
    assert_response :redirect

    assert_equal "Email sent with password reset instructions", flash[:info]
    token=session[:token]

    get :edit, params: {id: token, email: user.email}
    assert_response :success
    ...
    patch :update, params: {id: token, email: "reset_test@test.com", user: {password: 'dummy', password_confirmation: 'dummy'}}

    assert_response :redirect
    assert_redirected_to '/users/'+CGI.escape(user.callsign)
  ...
  test "request reset for wrong email" do
    ...
    post :create, params: {password_reset: {email: "bad@email.com"}}
    assert_response :success
  ...
  test "Can't reset to blank password or mismatch" do
    ...
    #blank passwords 
    patch :update, params: {id: token, email: "reset_test@test.com", user: {password: '', password_confirmation: ''}}
    assert_response :success
    assert_match "Password/confirmation can&#39;t be blank", @response.body

    #mismatching passwords
    patch :update, params: {id: token, email: "reset_test@test.com", user: {password: 'test1', password_confirmation: 'test2'}}
    assert_response :success
