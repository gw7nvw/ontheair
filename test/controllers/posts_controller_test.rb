...
class PostsControllerTest < ActionController::TestCase
  test "Can create new spot" do
    ...
    get :new, params: {topic_id: SPOT_TOPIC}
    ...
    #create post
    post :create, params: {topic_id: SPOT_TOPIC, pnp: 'off', post: {callsign: 'ZL4TEST', freq: '7.090', mode: 'AM', asset_codes: asset1.code+", "+asset2.code}}
    assert_response :success
    post=Post.last
  ...
  test "Can spot from asset" do
    ...
    get :new, params: {topic_id: SPOT_TOPIC, code: asset1.safecode}
    assert_response :success
    #fields
    assert_select '#post_callsign'
    assert_select '#post_freq'
    ...
    assert_select '#post_asset_codes' do |temp|
      assert_match %Q{value="#{asset1.code}"}, temp.to_s, "Default code applied"
    end
    assert_select '#pnp'
    assert_select "#submit", {value: "Create Post"}
  end
  ...
  test "Can respot from spot" do
    ...
    get :new, params: {topic_id: SPOT_TOPIC, spot: -spot.id}

    assert_response :success
    #fields
    assert_select '#post_callsign' do |temp|
      assert_match %Q{value="#{user2.callsign}"}, temp.to_s, "Activator callsign applied"
    end
    assert_select '#post_freq' do |temp|
      assert_match %Q{value="7.09"}, temp.to_s, "Freq applied"
    end
    assert_select '#post_mode' do |temp|
      assert_match %Q{value="SSB"}, temp.to_s, "Mode applied"
    end
    assert_select '#post_asset_codes' do |temp|
      assert_match %Q{value="#{asset1.code},#{asset2.code}"}, temp.to_s, "Default codes applied"
    end
    assert_select '#pnp'
    assert_select "#submit", {value: "Create Post"}
  end
  ...
  test "Can create new alert" do
    ...
    get :new, params: {topic_id: ALERT_TOPIC}
    assert_response :success
    ...
    userdate=thedate.in_time_zone(tz.name)
    #create post
    post :create, params: {topic_id: ALERT_TOPIC, pnp: 'off', post: {referenced_date: userdate.strftime('%Y-%m-%d'), referenced_time: userdate.strftime('%H:%M'),freq: '7.090', mode: 'AM', asset_codes: asset1.code+", "+asset2.code, duration: 1}}
    assert_response :success
    post=Post.last
    assert_select ".alert-success", /Posted/, "Success message"
