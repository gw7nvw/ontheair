...
class ContactsController < ApplicationController
  before_action :signed_in_user, only: %i[index show edit create new refute confirm]
  ...
  def create
    ...
    if signed_in?
      ...
      if (user.id === current_user.id) || current_user.is_admin
        ...
        result = @log.save
        if !result then
          flash[:error] = "Failed to create log: #{@log.errors.to_json}"
          render 'new'
          return
        end
        ...
        @contact.errors.add(:callsign1, "You do not have permission to use this callsign on this date")
        render 'new'
      end
    else
      redirect_to '/'
