...
def authenticated?(attribute, token)
 ...
 def bagged(asset_type = 'all')
  ...
  def activated_all(asset_type = 'all')
    if asset_type=='all' then
        ats=AssetType.where(keep_score: true)
        at_list=ats.map{|at| "'"+at.name+"'"}.join(",")
 ...
 # Sends youve been signed up choose a password email.
  ...
  def self.users_with_assets(sortby = "park", scoreby = "score", max_rows = 2000)
    ...
    users=User.find_by_sql [ "select * from users where id in ("+ids.uniq.map{|c| c.to_s}.join(",")+") order by cast(substring(SUBSTRING("+scoreby+" from '"+sortby+": [0-9]{1,9}') from ' [0-9]{1,9}') as integer) desc limit "+max_rows.to_s ]
  end
  ...
  def assets_by_type(asset_type, count_type)
    if asset_type=="qrp" then
      ...
      case count_type
      when 'activated'
        codes=self.activated_all(asset_type)
      when 'chased'
