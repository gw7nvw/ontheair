  ...
  def self.add_distance_to_assets
    ...
    ids.each do |id|
      ...
      while (tol < 2) && a.nil?
        ...
        ActiveRecord::Base.connection.execute("UPDATE assets a set nearest_road_id = (select r.id from roads r where ST_DWithin(ST_Centroid(r.linestring), a.boundary_simplified, #{tol}) ORDER BY ST_Distance(a.boundary_simplified, r.linestring) LIMIT 1)  where a.boundary is not null and a.id=#{id.id};")
        tol *= 10
        a = Asset.find(id).nearest_road_id
      end
      if a.nil?
        puts 'looking everywhere'
        ActiveRecord::Base.connection.execute("UPDATE assets a set nearest_road_id = (select r.id from roads r ORDER BY ST_Distance(a.boundary_simplified, r.linestring) LIMIT 1)  where a.boundary is not null and a.id=#{id.id};")
      end
      puts '.'
    end
    ...
    ids.each do |id|
      puts id.id.to_s + ': ' + id.code
      # Calculate distance from nearest road in meters
      ActiveRecord::Base.connection.execute("UPDATE assets a set road_distance = (SELECT ST_Distance(ST_Transform(a.boundary, 2193), ST_Transform(r.linestring,2193)) from roads r where r.id=a.nearest_road_id) where a.boundary is not null and a.id=#{id.id};")
    end
  end
end
