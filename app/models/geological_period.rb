class GeologicalPeriod < ActiveRecord::Base
require 'csv'
...
def era
  GeologicalEra.find_by_sql [" select * from geological_eras where start_mya<=#{self.start_mya} and end_mya>=#{self.end_mya}; "]
end
...
def self.import(filename)
  ...
  CSV.foreach(filename, :headers => true) do |row|
    place=row.to_hash
      ...
      p=GeologicalPeriod.new(newplace)
      puts p.to_json
      p.save
      puts p.id
    end
...
def self.from_date(date_mya)
  if date_mya!=nil then
    s=GeologicalPeriod.find_by("start_mya>=#{date_mya} and end_mya<#{date_mya}")
  end
