...
module MapHelper
  ...
  def get_map_x_y(gridx, gridy, z, xsize, ysize, id)
    ...
    c_x = ((gridx + 20_037_508) / (4891.97 * 2**(13 - z))).to_i
    c_y = ((gridy + 20_037_508) / (4891.97 * 2**(13 - z))).to_i
    system("mkdir /tmp/#{id}")
    ...
    lx = x * (4891.97 * 2**(13 - z)) - 20_037_508 # left border x grid ref of map image
    by = y * (4891.97 * 2**(13 - z)) - 20_037_508 # bottom border y grid ref of map image
  ...
  def get_map_zoomed(gridx, gridy, min_zoom, max_zoom, id)
    ...
    (min_zoom..max_zoom).step(2).each do |z|
      x = ((gridx + 19_740_000) / (2.356 * 2**(24 - z)))
      y = ((gridy + 19_680_000) / (2.356 * 2**(24 - z)))
